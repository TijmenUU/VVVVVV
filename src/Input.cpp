...
{
	for(SDL_GameControllerButton i = SDL_CONTROLLER_BUTTON_A; i < SDL_CONTROLLER_BUTTON_DPAD_UP;
 ...
 musicclass & music)
{
	...
	{
		//GAMEPAD TODO
		if(key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_s)
		 || key.controllerWantsRight(true))
			game.press_left = true;
		if(key.isDown(KEYBOARD_RIGHT) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_d) || key.isDown(KEYBOARD_w)
			...
			else
			{
				...
				else if(game.currentmenuname == "levellist")
				{
					...
					else
					{
						...
						std::string name
						 = game.saveFilePath + ed.ListOfMetaData[game.playcustomlevel].filename.substr(7) + ".vvv";
...
SDL_assert(0 && "Remove open level dir");
				...
				else if(game.currentmenuname == "playmodes")
				{
					...
					else if(game.currentmenuoption == 2 && game.unlock[17] && game.slowdown == 30
				...
				else if(game.currentmenuname == "timetrialcomplete3")
				{
				 ...
				 || game.currentmenuname == "nodeathmodecomplete2")
				{
					if(game.currentmenuoption == 0)
					{
						music.play(6);
		...
		if(game.currentmenuname == "controller" && game.currentmenuoption > 0 && game.currentmenuoption < 4
...
void gameinput(KeyPoll & key, Graphics & dwgfx, Game & game, mapclass & map, entityclass & obj, musicclass & music)
{
	...
	if(!script.running)
	{
		...
		if(key.isDown(KEYBOARD_z) || key.isDown(KEYBOARD_SPACE) || key.isDown(KEYBOARD_v) || key.isDown(KEYBOARD_UP)
		 || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_w) || key.isDown(KEYBOARD_s)
		 || key.isDown(game.controllerButton_flip))
		{
			game.press_action = true;
		};
    ...
    */
	...
	if(map.custommode && !map.custommodeforreal)
	{
		if((game.press_map || key.isDown(27)) && !game.mapheld)
		{
			...
			{
				if((int(std::abs(obj.entities[obj.getplayer()].vx)) <= 1)
				 && (int(obj.entities[obj.getplayer()].vy) == 0))
				{
 ...
 musicclass & music)
{
	...
	if(dwgfx.menuoffset == 0)
	{
		if(dwgfx.flipmode)
		{
			if(key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_a)
			 || key.isDown(KEYBOARD_s) || key.controllerWantsLeft(false))
				game.press_left = true;
			if(key.isDown(KEYBOARD_RIGHT) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_d) || key.isDown(KEYBOARD_w)
		...
		else
		{
			...
			if(key.isDown(KEYBOARD_RIGHT) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_d)
			 || key.isDown(KEYBOARD_s) || key.controllerWantsRight(false))
			{
				game.press_right = true;
			}
		}
	...
	if(!game.jumpheld)
	{
		...
		if(game.menupage == 3 && !game.gamesaved && game.press_action && !game.intimetrial && !game.nodeathmode
		 && !game.insecretlab && !game.inintermission)
		{
			game.flashlight = 5;
			game.screenshake = 10;
			music.playef(18);
	...
	if(!game.jumpheld)
	{
		...
		if(game.press_left)
		{
			...
			if(game.teleport_to_teleporter < 0)
				game.teleport_to_teleporter = map.numteleporters - 1;
		...
		{
			game.teleport_to_teleporter++;
			if(game.teleport_to_teleporter >= map.numteleporters)
