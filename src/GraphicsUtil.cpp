...
template<class T> void endian_swap(T * objp)
{
...
SDL_Surface * ScaleSurface(SDL_Surface * _surface, int Width, int Height, SDL_Surface * Dest)
{
	...
	float _stretch_factor_x = (static_cast<double>(Width) / static_cast<double>(_surface->w)),
		  _stretch_factor_y = (static_cast<double>(Height) / static_cast<double>(_surface->h));
	...
	for(Sint32 y = 0; y < _surface->h; y++)
		for(Sint32 x = 0; x < _surface->w; x++)
		{
			setRect(gigantoPixel,
			 static_cast<Sint32>((float(x) * _stretch_factor_x) - 1),
			 static_cast<Sint32>((float(y) * _stretch_factor_y) - 1),
			 static_cast<Sint32>(_stretch_factor_x + 1.0),
			 static_cast<Sint32>(_stretch_factor_y + 1.0));
...
void FillRect(SDL_Surface * _surface, SDL_Rect rect, int rgba) { SDL_FillRect(_surface, &rect, rgba); }

bool intersectRect(float left1,
 float right1,
 float bottom1,
 float top1,
 float left2,
