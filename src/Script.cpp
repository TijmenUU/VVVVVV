...
void scriptclass::run(KeyPoll & key,
 Graphics & dwgfx,
 Game & game,
 mapclass & map,
 entityclass & obj,
		...
		if(position < scriptlength)
		{
			...
			else if(words[0] == "speak_active")
			{
				...
				if(!game.backgroundtext)
				{
					...
					if(key.isDown(90) || key.isDown(32) || key.isDown(86) || key.isDown(KEYBOARD_UP)
			...
			else if(words[0] == "createcrewman")
			{
				...
				{
					obj
					 .createentity(game, ss_toi(words[1]), ss_toi(words[2]), 18, r, ss_toi(words[4]), ss_toi(words[5]));
				}
			}
 ...
 musicclass & music)
{
	switch(t)
	{
		...
		case 3:
			...
			if(dwgfx.setflipmode)
				dwgfx.flipmode = true; //set flipmode
			if(obj.nentity == 0)
			{
				obj.createentity(game, game.savex, game.savey, 0, 0); //In this game, constant, never destroyed
			}
