...
void entityclass::changecollect(int t, int s) { collect[t] = s; }

void entityclass::changecustomcollect(int t, int s) { collect[t] = s; }

void entityclass::changeflag(int t, int s) { flags[t] = s; }
...
void entityclass::createentity(Game & game,
 float xp,
 float yp,
 int t,
 float vx /*= 0*/,
	...
	switch(t)
	{
        ...
        */
		...
		case 56: //Custom enemy
			...
			{
				case 0:
					setenemyroom(k, 4 + 100, 0 + 100);
					break;
				case 1:
					setenemyroom(k, 2 + 100, 0 + 100);
					break;
				case 2:
					setenemyroom(k, 12 + 100, 3 + 100);
					break;
				case 3:
					setenemyroom(k, 13 + 100, 12 + 100);
					break;
				case 4:
					setenemyroom(k, 16 + 100, 9 + 100);
					break;
				case 5:
					setenemyroom(k, 19 + 100, 1 + 100);
					break;
				case 6:
					setenemyroom(k, 19 + 100, 2 + 100);
					break;
				case 7:
					setenemyroom(k, 18 + 100, 3 + 100);
					break;
				case 8:
					setenemyroom(k, 16 + 100, 0 + 100);
					break;
				case 9:
					setenemyroom(k, 14 + 100, 2 + 100);
					break;
				default:
					setenemyroom(k, 4 + 100, 0 + 100);
					break;
...
bool entityclass::entitycollide(int a, int b)
{
	...
	if(UtilityClass::intersects(temprect, temprect2))
		return true;
	return false;
}
		...
		if(entities[t].yp + entities[t].cy <= entities[l].yp + entities[l].h)
		{
			...
			if(entities[l].xp < 160)
			{
				...
				if(entities[t].xp + entities[t].cx + 1 < entities[l].xp + 10)
					linetemp++;
				if(entities[t].xp + entities[t].cx + 1 + entities[t].w < entities[l].xp + 10)
					linetemp++;
				if(entities[t].oldxp + entities[t].cx + 1 < entities[l].xp + 10)
					linetemp++;
				if(entities[t].oldxp + entities[t].cx + 1 + entities[t].w < entities[l].xp + 10)
					linetemp++;
...
void entityclass::customwarplinecheck(int i)
{
	...
	if(entities[i].active)
	{
		...
		for(int j = 0; j < nentity; j++)
		{
			...
			{ //Active
				if(entities[i].rule == 0 && entities[j].rule == 5)
				{ //Player vs vertical line!
				...
				if(entities[i].rule == 0 && entities[j].rule == 7)
				{ //Player vs horizontal WARP line
							...
							if(entities[i].size == 0 && (entities[j].size == 0 || entities[j].size == 12))
							{
								...
								if(dwgfx.flipmode)
								{
									if(dwgfx.Hitest(dwgfx.flipsprites[entities[i].drawframe],
										colpoint1,
										dwgfx.flipsprites[entities[j].drawframe],
										colpoint2))
									{
								...
								else
								{
									if(dwgfx.Hitest(dwgfx.sprites[entities[i].drawframe],
										colpoint1,
										dwgfx.sprites[entities[j].drawframe],
										colpoint2))
									{
										//Do the collision stuff
										game.deathseq = 30;
					...
					if(game.supercrewmate)
					{
						...
						if(entities[i].type == 14) //i is the supercrewmate
						{
							...
							if(entities[j].type == 8
							 && entities[j].rule == 3) //Entity to entity (well, checkpoints anyway!)
							{
								if(entities[j].onentity > 0)
								{
									if(entitycollide(i, j))
