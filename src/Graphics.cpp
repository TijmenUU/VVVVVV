	...
	for(unsigned int i = 0; i < _s.length(); i++)
	{
        ...
        */
		...
		if(flipmode)
		{
			...
			SDL_Rect printrect
			 = {Sint16((_x) + bfontpos), Sint16(_y), Sint16(bfont_rect.w * sc), Sint16(bfont_rect.h * sc)};
		...
		else
		{
			...
			SDL_Rect printrect = {static_cast<Sint16>((_x) + bfontpos),
			 ...
			 static_cast<Sint16>((bfont_rect.h * sc) + 1)};
			BlitSurfaceColoured(tempPrint, NULL, backBuffer, &printrect, ct);
			SDL_FreeSurface(tempPrint);
		}
				...
				if(flipmode)
				{
					for(j = 0; j < textbox[i].numlines; j++)
					{
						Print(textbox[i].xp + 8,
						 textbox[i].yp + (textbox[i].numlines * 8) - (j * 8),
						 textbox[i].line[j],
						 196,
						 196,
						 255 - help.glow);
...
void Graphics::textboxtimer(int t) { textbox[m].timer = t; }

void Graphics::addline(std::string t) { textbox[m].addline(t); }

void Graphics::textboxadjust() { textbox[m].adjust(); }
...
void Graphics::createtextbox(std::string t, int xp, int yp, int r /*= 255*/, int g /*= 255*/, int b /*= 255*/)
{
	...
	if(m < 20)
	{
		...
		textbox[m].xp = xp;
		if(xp == -1)
			textbox[m].xp = 160 - (((t.length() / 2) + 1) * 8);
		textbox[m].yp = yp;
	...
	for(int i = 0; i < game.nummenuoptions; i++)
	{
		if(i == game.currentmenuoption)
		{
			...
			if(game.menuoptionsactive[i])
			{
				...
				Print(110 + (i * division) - 16 + game.menuxoff,
				 140 + (i * 12) + game.menuyoff,
				 tempstring,
				 cr,
				 cg,
				 cb);
		...
		else
		{
			...
			if(game.menuoptionsactive[i])
			{
				Print(110 + (i * division) + game.menuxoff,
				 140 + (i * 12) + game.menuyoff,
				 game.menuoptions[i],
				 cr,
				 cg,
		...
		if(!obj.entities[i].invis && obj.entities[i].active)
		{
			...
			else if(obj.entities[i].size == 3) // Big chunky pixels!
			{
				...
				else if(obj.entities[i].colour == 2)
				{
					...
					FillRect(backBuffer,
					 prect,
					 int(160 - help.glow / 2 - (fRandom() * 20)),
					 200 - help.glow / 2,
					 220 - help.glow);
				}
			}
			...
			{
				drawtele(obj.entities[i].xp,
				 obj.entities[i].yp,
				 obj.entities[i].drawframe,
				 obj.entities[i].colour,
				 help);
			}
			...
			else if(obj.entities[i].size == 13)
			{
				...
				if(flipmode)
				{
					...
					SDL_Rect drawRect
					 = {Sint16(obj.entities[i].xp), Sint16(obj.entities[i].yp), sprites_rect.x, sprites_rect.y};
					SDL_Surface * TempSurface
					 = ScaleSurface(flipsprites[obj.entities[i].drawframe], 6 * sprites_rect.w, 6 * sprites_rect.w);
				...
				else
				{
					...
					SDL_Rect drawRect = {Sint16(obj.entities[i].xp),
					 Sint16(obj.entities[i].yp),
					 Sint16(sprites_rect.x * 6),
					 Sint16(sprites_rect.y * 6)};
					SDL_Surface * TempSurface
					 = ScaleSurface(flipsprites[obj.entities[i].drawframe], 6 * sprites_rect.w, 6 * sprites_rect.h);
					BlitSurfaceColoured(TempSurface, NULL, backBuffer, &drawRect, ct);
					SDL_FreeSurface(TempSurface);
				}
			}
	...
	if(!foregrounddrawn)
	{
			...
			for(j = 0; j < 29 + map.extrarow; j++)
			{
				for(int i = 0; i < 40; i++)
				{
					if(map.contents[i + map.vmult[j]] > 0)
						drawforetile(i * 8, j * 8, map.contents[i + map.vmult[j]]);
				}
			}
		}
			...
			for(int jt = 0; jt < 29 + map.extrarow; jt++)
			{
				for(int it = 0; it < 40; it++)
				{
					if(map.contents[it + map.vmult[jt]] > 0)
						drawforetile2(it * 8, jt * 8, map.contents[it + map.vmult[jt]]);
				}
			}
		}
		else if(map.tileset == 2)
		{
			for(j = 0; j < 29 + map.extrarow; j++)
			{
				...
				{
					if(map.contents[i + map.vmult[j]] > 0)
						drawforetile3(i * 8, j * 8, map.contents[i + map.vmult[j]], map.rcol);
				}
			}
		}
...
void Graphics::drawtowermap(mapclass & map)
{
	...
	for(int j = 0; j < 30; j++)
	{
		for(int i = 0; i < 40; i++)
		{
			...
			if(temp > 0)
				drawtile3(i * 8, (j * 8) - ((int)map.ypos % 8), temp, map.colstate);
		}
	}
}
		...
		if(!obj.entities[i].invis && obj.entities[i].active)
		{
			...
			else if(obj.entities[i].size == 3) // Big chunky pixels!
			{
				...
				else if(obj.entities[i].colour == 2)
				{
					FillRect(backBuffer,
					 prect,
					 getRGB(160 - help.glow / 2 - (fRandom() * 20), 200 - help.glow / 2, 220 - help.glow));
				}
			}
			else if(obj.entities[i].size == 4) // Small pickups
			{
				drawhuetile(obj.entities[i].xp,
				 obj.entities[i].yp - map.ypos,
				 obj.entities[i].tile,
				 obj.entities[i].colour);
			}
			else if(obj.entities[i].size == 5) //Horizontal Line
...
void Graphics::textboxcenterx() { textbox[m].centerx(); }

int Graphics::textboxwidth() { return textbox[m].w; }
...
void Graphics::textboxmoveto(int xo) { textbox[m].xp = xo; }

void Graphics::textboxcentery() { textbox[m].centery(); }
...
void Graphics::flashlight() { FillRect(backBuffer, 0xBBBBBBBB); }
...
void Graphics::bigrprint(int x, int y, std::string & t, int r, int g, int b, bool cen, float sc)
{
	...
	for(size_t i = 0; i < t.length(); i++)
	{
		...
		if(flipmode)
		{
			...
			SDL_Rect printrect
			 = {Sint16(x + bfontpos), Sint16(y), Sint16(bfont_rect.w * sc), Sint16(bfont_rect.h * sc)};
			BlitSurfaceColoured(tempPrint, NULL, backBuffer, &printrect, ct);
			SDL_FreeSurface(tempPrint);
		}
		else
...
Uint32 Graphics::getRGB(Uint8 r, Uint8 g, Uint8 b) { return SDL_MapRGB(backBuffer->format, b, g, r); }

Uint32 Graphics::getBGR(Uint8 r, Uint8 g, Uint8 b) { return SDL_MapRGB(backBuffer->format, r, g, b); }

Uint32 Graphics::getRGB(Uint32 _col) { return (_col); }

Uint32 Graphics::RGBflip(Uint8 r, Uint8 g, Uint8 b) { return SDL_MapRGB(backBuffer->format, r, g, b); }
...
void Graphics::setcolreal(Uint32 t) { ct.colour = t; }
...
bool Graphics::onscreen(int t) { return (t >= -40 && t <= 280); }
