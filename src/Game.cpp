			...
			for(pElem = hRoot.FirstChild("Data").FirstChild().Element(); pElem; pElem = pElem->NextSiblingElement())
			{
				...
				if(pKey == "customlevelscore")
				{
					...
					if(TextString.length())
					{
						...
						{
							if(i < 200)
								customlevelscore[i] = (atoi(values[i].c_str()));
						}
					}
				}
            ...
            */
			...
			case 82:
				...
				if(timetrialrank > bestrank[timetriallevel] || bestrank[timetriallevel] == -1)
				{
					...
					{
						if(timetriallevel == 0)
							NETWORK_unlockAchievement("vvvvvvtimetrial_station1_fixed");
						if(timetriallevel == 1)
							NETWORK_unlockAchievement("vvvvvvtimetrial_lab_fixed");
						if(timetriallevel == 2)
							NETWORK_unlockAchievement("vvvvvvtimetrial_tower_fixed");
						if(timetriallevel == 3)
							NETWORK_unlockAchievement("vvvvvvtimetrial_station2_fixed");
						if(timetriallevel == 4)
							NETWORK_unlockAchievement("vvvvvvtimetrial_warp_fixed");
						if(timetriallevel == 5)
							NETWORK_unlockAchievement("vvvvvvtimetrial_final_fixed");
					}
				}
			...
			case 1001:
				...
				if(dwgfx.flipmode)
				{
					...
					{
						dwgfx.createtextbox(
						 " " + help.number(trinkets) + " out of " + help.number(map.customtrinkets) + " ",
						 50,
						 65,
						 174,
						 174,
			...
			case 1011:
				...
				if(dwgfx.flipmode)
				{
					...
					{
						dwgfx.createtextbox("    " + help.number(int(map.customcrewmates - crewmates)) + " remains    ",
						 50,
						 135,
						 174,
						 174,
...
std::string Game::partimestring(UtilityClass & help)
{
	...
	{
		tempstring
		 = help.twodigits(int((timetrialpar - (timetrialpar % 60)) / 60)) + ":" + help.twodigits(timetrialpar % 60);
	}
	else
	{
		...
		else
		{
			...
			for(int i = 0; i < (int)ed.ListOfMetaData.size(); i++) // FIXME: int/size_t! -flibit
			{
				if(i >= levelpage * 8 && i < (levelpage * 8) + 8)
				{
					...
					std::transform(menuoptions[tcount].begin(),
					 menuoptions[tcount].end(),
					 menuoptions[tcount].begin(),
					 ::tolower);
					tcount++;
				}
			}
